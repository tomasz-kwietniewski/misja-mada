...
function ae_birth_pl(?string $date): string {
 ...
 */
function ae_mail_child(array $donor, array $child, string $personalNote): bool {
    ...
    $row('Data urodzenia', ae_birth_pl($child['birth_date'] ?? null));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (!empty($_POST['notify_child']) && $childId !== null) {
            ...
            if ($dn && $ch && ae_mail_child($dn, $ch, trim((string)($_POST['personal_note'] ?? '')))) {
